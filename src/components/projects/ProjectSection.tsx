...
export default function ProjectSection() {
  ...
  return (
    <div
      id="projects"
      className="flex flex-col w-full p-5 sm:p-10 justify-center lg:justify-start"
    >
      <Heading>Projects</Heading>

      <Tabs
        defaultValue="All"
        className="w-full max-w-[350px] md:max-w-[750px]"
      >
        <TabsList>
          {categories.map((cat) => (
            <TabsTrigger
        ...
        {categories.map((cat) => {
          ...
          const filteredProjects =
            cat === "All"
              ? projects
              : projects.filter((p) => p.type.includes(cat));

          return (
            <TabsContent key={cat} value={cat}>
              <Flex direction="column" align="start" gap="6" className="mt-5">
                {filteredProjects.length > 0 && (
                  <ProjectCard data={filteredProjects[0]} />
                )}
