    ...
    return(
        <Flex direction="column">
            <Flex
                align="center"
                justify="between"
                className="cursor-pointer text-white bg-container p-4 rounded-md hover:bg-opacity-80 transition-all duration-300"
                onClick={props.onToggle}
            >    
                <Text>{data.title}</Text>
                <ChevronButton isOpen={props.isOpen}/>
            </Flex>
            {/* <br/> */}
            {/* COLLAPSIBLE CONTENT */}
            <div
                className={`overflow-hidden transition-all duration-500 ease-in-out ${
                props.isOpen ? "max-h-[1000px]" : "max-h-0"
                }`}
            >
                <div 
