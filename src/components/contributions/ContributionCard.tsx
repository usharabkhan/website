...
export default function ContributionCard({ data }: { data: Contribution }) {
  return (
    <Link to={data.link}>
      <Card className="w-full max-w-sm p-5 rounded-2xl bg-black border border-transparent hover:border-secondary transition-all duration-300">
        {/* Header */}
        <div className="flex items-center gap-3 mb-4">
          <img src={data.logo} alt={data.title} className="h-10 w-10 rounded-full object-cover" />
          <div className="flex flex-col text-white">
            <Text weight="medium" size={"4"} className="text-secondary">{data.role}</Text>
